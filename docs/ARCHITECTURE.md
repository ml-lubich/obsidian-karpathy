...
├── llm.py            — OpenAI-compatible chat adapter with vault context prompts
├── models.py         — Dataclasses for Note, Node, Edge, and KnowledgeGraph
└── web.py            — FastAPI app factory: graph API, LLM API, health endpoint, SPA hosting

web/
└── src/              — Bun/Vite React frontend for graph, inspector, and AI chat
```
...
5. `web.py` serialises the graph to JSON, exposes LLM status/chat endpoints, and serves the built React app or packaged fallback UI.
